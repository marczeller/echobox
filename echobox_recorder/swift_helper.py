...
def helper_self_test() -> bool:
    """Run the helper in test-signal mode for 1 second and verify the WAV file.

    Returns True on success. Used by `echobox status` and CI.
    """
    binary = find_helper_binary()
    if binary is None:
        return False
    import tempfile
    import wave
    ...
    with tempfile.TemporaryDirectory() as tmp:
        session_dir = Path(tmp) / "self-test"
        argv = [
            str(binary),
            "--session-dir", str(session_dir),
            "--source", "test-signal",
        ...
        with wave.open(str(wav_path)) as w:
            ...
            if w.getnchannels() != 1:
                return False
            if w.getnframes() < 8000:  # at least ~0.5s of audio
                return False
        return True
